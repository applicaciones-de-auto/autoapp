...
//    private int pnMobile, pnEmail, pnSocMed, pnAddress, pnContact = 0;
    ...
    private void initTextFieldPattern() {
        Pattern numberOnly, textOnly, suffOnly;
        numberOnly = Pattern.compile("[0-9-]*");
        textOnly = Pattern.compile("[A-Za-z ]*");
        suffOnly = Pattern.compile("[A-Za-z .]*");
        txtField13.setTextFormatter(new InputTextFormatterUtil(numberOnly));
        ...
        txtField05.setTextFormatter(new InputTextFormatterUtil(textOnly));
                ...
                switch (txtFieldID) {
                    ...
                    case "txtField25":
                        ...
                        if ("success".equals(loJSON.get("result"))) {
                            ...
                            System.out.println("souse Id" + oTrans.getModel().getModel().getSpouseID());
                            System.out.println("souse Name" + oTrans.getModel().getModel().getSpouseNm());
                        } else {
                            ShowMessageFX.Warning(null, pModuleName, (String) loJSON.get("message"));
                            txtField25.setText("");
        ...
        switch (lsButton) {
            case "btnAdd":
                ...
                if ("success".equals((String) poJson.get("result"))) {
                    ...
                    ShowMessageFX.Warning(null, pModuleName, (String) poJson.get("message"));
                }
                break;
            case "btnEdit":
            ...
            case "btnSave":
                ...
                if (setSelection()) {
                    ...
                    if ("success".equals((String) poJson.get("result"))) {
                        ShowMessageFX.Information(null, "Customer Information", (String) poJson.get("message"));
                        poJson = oTrans.openRecord(oTrans.getModel().getModel().getClientID());
                        if ("success".equals((String) poJson.get("result"))) {
                            loadCustomerInformation();
                            loadAddress();
                            loadContact();
            ...
            case "btnTabRem":
                if (pnRow < 0) {
                    ShowMessageFX.Warning(getStage(), "No selected item!", "Warning", pModuleName);
                    return;
                }
                switch (iTabIndex) {
                    case 0: // Address
    ...
    private boolean setSelection() {
        if (pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE) {
            if (comboBox18.getSelectionModel().getSelectedIndex() < 0) {
                ShowMessageFX.Warning("No `Client Type` selected.", "Customer Client Type", "Please select `Client Type` value.");
                clearFields();
                clearTables();
                return false;
            } else {
                ...
                if (comboBox18.getSelectionModel().getSelectedIndex() == 0) {
                    if (comboBox07.getSelectionModel().getSelectedIndex() < 0) {
                        ShowMessageFX.Warning("No `Title` selected.", "Customer Title", "Please select `Title` value.");
                    ...
                    if (comboBox08.getSelectionModel().getSelectedIndex() < 0) {
                        ShowMessageFX.Warning("No `Gender` selected.", "Customer Gender", "Please select `Gender` value.");
                    ...
                    if (comboBox09.getSelectionModel().getSelectedIndex() < 0) {
                        ShowMessageFX.Warning("No `Civil Status` selected.", "Customer Civil Status", "Please select `Civil Status` value.");
        ...
        if (pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE) {
            ...
            tblAddress.setOnKeyReleased((KeyEvent t) -> {
                ...
                switch (key) {
                    ...
                    case UP:
                        int pnRows = 0;
                        pnRows = tblAddress.getSelectionModel().getSelectedIndex();
                        pnRow = pnRows;
                        break;
                    default:
                        return;
                }
                if (oTrans.getAddress(pnRow, "sAddrssID").toString().isEmpty()) {
                    btnTabRem.setVisible(true);
                } else {
                    btnTabRem.setVisible(false);
                }
            ...
            tblContact.setOnKeyReleased((KeyEvent t) -> {
                ...
                switch (key) {
                    ...
                    case UP:
                        int pnRows = 0;
                        int x = -1;
                        pnRows = tblContact.getSelectionModel().getSelectedIndex();
                        pnRow = pnRows;
                        break;
                    default:
                        return;
                }
                if (oTrans.getMobile(pnRow, "sMobileID").toString().isEmpty()) {
                    btnTabRem.setVisible(true);
                } else {
                    btnTabRem.setVisible(false);
                }
            ...
            tblEmail.setOnKeyReleased((KeyEvent t) -> {
                ...
                switch (key) {
                    ...
                    case UP:
                        int pnRows = 0;
                        int x = -1;
                        pnRows = tblEmail.getSelectionModel().getSelectedIndex();
                        pnRow = pnRows;
                        break;
                    default:
                        return;
                }
                if (oTrans.getEmail(pnRow, "sEmailIDx").toString().isEmpty()) {
                    btnTabRem.setVisible(true);
                } else {
                    btnTabRem.setVisible(false);
            ...
            tblSocMed.setOnKeyReleased((KeyEvent t) -> {
                ...
                switch (key) {
                    ...
                    case UP:
                        int pnRows = 0;
                        int x = -1;
                        pnRows = tblSocMed.getSelectionModel().getSelectedIndex();
                        pnRow = pnRows;
                        break;
                    default:
                        return;
                }
                if (oTrans.getSocialMed(pnRow, "sSocialID").toString().isEmpty()) {
                    btnTabRem.setVisible(true);
                } else {
                    btnTabRem.setVisible(false);
